...
class Dataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        while True: # choose another video randomly.
            another_vid = random.choice(self.vid_list)
            if another_vid != vid or len(self.vid_list) <= 1:
                break
        rand_shid = np.random.randint(data.shot_num[another_vid]-self.clip_len)
        shids_right = list(range(rand_shid, rand_shid+(self.clip_len-pseudo_bound)))
        clips = self.load_clip(vid, shids_left) + self.load_clip(another_vid, shids_right)
